...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.patch<{
    ...
    if (body.firstName !== undefined) {
      const value = optionalString(body.firstName, MAX_NAME_LENGTH);
      if (!value) return reply.code(400).send({ error: "INVALID_FIRST_NAME" });
      data.firstName = value;
    }
    if (body.lastName !== undefined) {
      const value = optionalString(body.lastName, MAX_NAME_LENGTH);
      if (!value) return reply.code(400).send({ error: "INVALID_LAST_NAME" });
      data.lastName = value;
    }
    if (body.email !== undefined) {
      const value = optionalString(body.email, MAX_EMAIL_LENGTH);
    ...
    if (body.phone !== undefined) {
      const value = optionalString(body.phone, MAX_PHONE_LENGTH);
      if (value === undefined) return reply.code(400).send({ error: "INVALID_PHONE" });
      data.phone = value;
    }
    if (body.avatarUrl !== undefined) {
      const value = optionalString(body.avatarUrl, MAX_URL_LENGTH);
      if (value === undefined) return reply.code(400).send({ error: "INVALID_AVATAR_URL" });
      data.avatarUrl = value;
    }
    if (body.preferredLanguage !== undefined) {
      const value = optionalString(body.preferredLanguage, MAX_LANGUAGE_LENGTH);
    ...
    if (company) {
      const companyName = optionalString(company.companyName, MAX_COMPANY_NAME_LENGTH);
      ...
      for (const [key, value] of Object.entries({
        ...
        website: optionalString(company.website, MAX_URL_LENGTH),
        address: optionalString(company.address, MAX_ADDRESS_LENGTH),
    ...
    if (preferences) {
      ...
      if (preferences.locale !== undefined && !optionalString(preferences.locale, MAX_LANGUAGE_LENGTH)) return reply.code(400).send({ error: "INVALID_LOCALE" });
    ...
    try {
      const user = await prisma.$transaction(async (tx) => {
        ...
        if (company) {
          ...
          const companyName = optionalString(company.companyName, MAX_COMPANY_NAME_LENGTH) ?? existing?.companyName;
          ...
          const companyData = {
            ...
            website: optionalString(company.website, MAX_URL_LENGTH) ?? (company.website === null ? null : existing?.website),
            address: optionalString(company.address, MAX_ADDRESS_LENGTH) ?? (company.address === null ? null : existing?.address),
          ...
          await tx.companyProfile.upsert({
            ...
            create: { userId: request.user!.id, ...companyData },
            update: companyData,
          });
        }
        ...
        if (preferences) {
          await tx.userPreference.upsert({
            ...
            create: {
              ...
              locale: preferences.locale ?? "tr-TR",
  ...
  app.put<{
    Body: { regions: Array<{ country?: string; city: string; district?: string | null; radiusKm?: number | null }> };
  }>("/v1/users/me/work-regions", { preHandler: requireAuth }, async (request, reply) => {
    const regions = request.body?.regions;
    if (!Array.isArray(regions) || regions.length > MAX_REGION_COUNT) return reply.code(400).send({ error: "INVALID_REGIONS" });
    ...
    for (const region of regions) {
      ...
      if (country.length !== 2 || !city || (region.district !== undefined && district === undefined) || (radiusKm !== undefined && (!Number.isFinite(radiusKm) || radiusKm < 1 || radiusKm > 1000))) {
        return reply.code(400).send({ error: "INVALID_REGION" });
      }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (normalized.length > 0) await tx.workRegion.createMany({ data: normalized.map((region) => ({ userId: request.user!.id, ...region })) });
    });
    const workRegions = await prisma.workRegion.findMany({ where: { userId: request.user!.id }, orderBy: [{ country: "asc" }, { city: "asc" }] });
    return { workRegions };
  });
  ...
  app.post<{ Body: { currentPassword: string; newPassword: string } }>("/v1/users/me/password", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (newPassword.length < 8 || newPassword.length > 128 || currentPassword.length === 0) return reply.code(400).send({ error: "INVALID_PASSWORD" });
  ...
  app.get("/v1/users/count", async () => {
    const count = await prisma.user.count({ where: { status: { not: "DELETED" } } });
    return { count };
  });
}
