...
export async function updateOrder(accessToken: string, id: string, input: Partial<Omit<Order, "id" | "createdAt" | "updatedAt">> & { budgetMinor?: number | string; payload?: OrderRequirements }) { return request<{ order: Order }>(`/v1/orders/${id}`, { ...auth(accessToken), method: "PATCH", body: JSON.stringify(input) }); }
