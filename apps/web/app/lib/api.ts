...
async function request<T>(path: string, init?: RequestOptions): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(body.message ?? body.error ?? `REQUEST_FAILED_${response.status}`);
  }
...
export function browserAccessToken() {
  return typeof window === "undefined" ? null : window.localStorage.getItem("yuklab_access_token");
}

export function browserRole() {
  return typeof window === "undefined" ? null : window.localStorage.getItem("yuklab_user_role");
}
...
export type CompanyProfile = { id: string; companyName: string; taxNumber?: string | null; website?: string | null; address?: string | null; city?: string | null; district?: string | null; verificationStatus: string };
export type UserPreference = { theme: string; locale: string; pushEnabled: boolean; marketingEnabled: boolean };
export type WorkRegion = { id: string; country: string; city: string; district?: string | null; radiusKm?: string | null };
export type ProfileUser = AuthUser & { companyProfile?: CompanyProfile | null; preferences?: UserPreference | null; driverProfile?: Record<string, unknown> | null; serviceProvider?: Record<string, unknown> | null; workRegions?: WorkRegion[] };
export type LoginResponse = { accessToken: string; refreshToken: string; expiresIn: number; user: AuthUser };
export type OrderRequirements = { weightKg?: number; volumeM3?: number; vehicleType?: string; vehicleSubtype?: string; refrigerated?: boolean; loadType?: string; fragile?: boolean; notes?: string; dimensions?: string; deliveryDate?: string };
export type Order = { id: string; customerId?: string; assignedDriverId?: string | null; serviceType: string; status: string; pickupAddress: string; deliveryAddress?: string | null; pickupLat?: string | null; pickupLng?: string | null; deliveryLat?: string | null; deliveryLng?: string | null; scheduledAt?: string | null; budgetMinor?: string | null; currency: string; urgency?: number; payload?: OrderRequirements | null; createdAt: string; updatedAt?: string };
...
export type NotificationItem = { id: string; type: string; title: string; body: string; data?: unknown; readAt?: string | null; createdAt: string };
...
export async function register(input: { email?: string; phone?: string; password: string; firstName: string; lastName: string; preferredLanguage?: string }) {
  return request<{ user: AuthUser }>("/v1/auth/register", { method: "POST", body: JSON.stringify(input) });
}

export async function refresh(refreshToken: string) {
  return request<{ accessToken: string; refreshToken: string; expiresIn: number }>("/v1/auth/refresh", { method: "POST", body: JSON.stringify({ refreshToken }), retryOn401: false });
}

export async function logout(accessToken: string, refreshToken: string) {
  await request<void>("/v1/auth/logout", { ...auth(accessToken), method: "POST", body: JSON.stringify({ refreshToken }) });
  clearBrowserSession();
}

export async function becomeProvider(accessToken: string, category = "GENERAL", providerType: "DRIVER" | "SERVICE_PROVIDER" = "DRIVER") {
  const result = await request<{ user: AuthUser; provider: { type: string; category: string | null; isOnline: boolean; isAvailable: boolean } }>("/v1/auth/become-provider", { ...auth(accessToken), method: "POST", body: JSON.stringify({ category, providerType }) });
  if (typeof window !== "undefined") window.localStorage.setItem("yuklab_user_role", result.user.role);
  return result;
}

export async function getProfile(accessToken: string) { return request<{ user: ProfileUser }>("/v1/users/me", auth(accessToken)); }
export async function updateProfile(accessToken: string, input: Record<string, unknown>) { return request<{ user: ProfileUser }>("/v1/users/me", { ...auth(accessToken), method: "PATCH", body: JSON.stringify(input) }); }
export async function updateWorkRegions(accessToken: string, regions: Array<{ country?: string; city: string; district?: string | null; radiusKm?: number | null }>) { return request<{ workRegions: WorkRegion[] }>("/v1/users/me/work-regions", { ...auth(accessToken), method: "PUT", body: JSON.stringify({ regions }) }); }
export async function changePassword(accessToken: string, currentPassword: string, newPassword: string) { return request<void>("/v1/users/me/password", { ...auth(accessToken), method: "POST", body: JSON.stringify({ currentPassword, newPassword }) }); }
export async function deactivateAccount(accessToken: string) { return request<void>("/v1/users/me", { ...auth(accessToken), method: "DELETE" }); }
...
export async function getOrder(accessToken: string, id: string) { return request<{ order: Order }>(`/v1/orders/${id}`, auth(accessToken)); }
export async function updateOrder(accessToken: string, id: string, input: Partial<Omit<Order, "id" | "createdAt" | "updatedAt" | "budgetMinor" | "payload">> & { budgetMinor?: number | string; payload?: OrderRequirements }) { return request<{ order: Order }>(`/v1/orders/${id}`, { ...auth(accessToken), method: "PATCH", body: JSON.stringify(input) }); }
export async function cancelOrder(accessToken: string, id: string) { return request<void>(`/v1/orders/${id}`, { ...auth(accessToken), method: "DELETE" }); }
...
export async function updateOffer(accessToken: string, orderId: string, offerId: string, input: { amountMinor?: number; etaMinutes?: number | null; note?: string | null; expiresAt?: string | null }) { return request<{ offer: Offer }>(`/v1/orders/${orderId}/offers/${offerId}`, { ...auth(accessToken), method: "PATCH", body: JSON.stringify(input) }); }
export async function withdrawOffer(accessToken: string, orderId: string, offerId: string) { return request<void>(`/v1/orders/${orderId}/offers/${offerId}`, { ...auth(accessToken), method: "DELETE" }); }
...
export async function listNotifications(accessToken: string, unreadOnly = false) { return request<{ notifications: NotificationItem[]; unreadCount: number }>(`/v1/notifications?unreadOnly=${unreadOnly}`, auth(accessToken)); }
export async function markNotificationRead(accessToken: string, id: string) { return request<{ notification: NotificationItem }>(`/v1/notifications/${id}/read`, { ...auth(accessToken), method: "PATCH", body: "{}" }); }
export async function markAllNotificationsRead(accessToken: string) { return request<{ updated: number }>("/v1/notifications/read-all", { ...auth(accessToken), method: "POST", body: "{}" }); }
...
export async function checkApiHealth() { return request<{ status: string; service: string; version?: string }>("/health", { retryOn401: false }); }
