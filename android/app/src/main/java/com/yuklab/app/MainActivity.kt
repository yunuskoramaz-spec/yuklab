...
class MainActivity : Activity() {
    ...
    private fun showHome() {
        val b = base("YÜKLAB")
        text(b,"Akıllı lojistik ağı",28,true)
        text(b,"Yük taşıma, kurye ve acil yardım işlemlerini tek Android uygulamasından yönet.",16,false)
        button(b,"Taşıma talebi oluştur") { showCreateOrder() }
        button(b,"Siparişlerim") { showOrders() }
        button(b,"Provider Hub") { showProvider() }
        button(b,"Ayarlar") { showSettings() }
        if (token == null) button(b,"Giriş / Kayıt") { showLogin() } else {
            text(b,"Oturum açık",14,true)
            button(b,"Çıkış yap") { prefs.edit().remove("token").apply(); token=null; showHome() }
        }
    }

    private fun showLogin() {
        val b = base("Giriş")
        val identifier = input(b,"E-posta veya telefon")
        val password = input(b,"Şifre", true)
        button(b,"Giriş yap") {
            if (server.isBlank()) { toast("Önce Ayarlar > Sunucu adresinden API adresini girin."); return@button }
    ...
    private fun showCreateOrder() {
        ...
        button(b,"Talep oluştur") {
            ...
            val payload=JSONObject().apply{if(weight.text.isNotBlank())put("weightKg",weight.text.toString().toDoubleOrNull());if(volume.text.isNotBlank())put("volumeM3",volume.text.toString().toDoubleOrNull());put("vehicleType",vehicle.selectedItem.toString());put("refrigerated",refrigerated.isChecked)}
            val body=JSONObject().put("serviceType",service.selectedItem.toString()).put("pickupAddress",pickup.text.toString()).put("deliveryAddress",delivery.text.toString()).put("currency","TRY").put("payload",payload)
            if(budget.text.isNotBlank()) body.put("budgetMinor",((budget.text.toString().toDoubleOrNull()?:0.0)*100).toLong())
            request("/v1/orders","POST",body,token){ok,data->if(ok){toast("Talep oluşturuldu");showOrders()}else toast(data.optString("error","Talep oluşturulamadı"))}
        }
    ...
    private fun showMatches(orderId:String){
        ...
        request("/v1/orders/$orderId/matches","GET",null,token){ok,data->runOnUiThread{if(!ok){toast(data.optString("error","Eşleşme alınamadı"));return@runOnUiThread};val arr=data.optJSONArray("matches")?:return@runOnUiThread;for(i in 0 until arr.length()){val m=arr.getJSONObject(i);text(b,"Skor: ${m.optDouble("score")}  •  ${m.optDouble("distanceKm")} km  •  Puan: ${m.optDouble("rating")}\nAraç: ${m.optString("vehicleType","-")} ${m.optString("vehicleSubtype","")}",16,false)}}}
        button(b,"Geri"){showOrders()}
    }
    ...
    private fun text(parent:LinearLayout,value:String,size:Float,bold:Boolean){val v=TextView(this).apply{text=value;textSize=size;setTextColor(Color.rgb(25,25,25));if(bold)setTypeface(null,android.graphics.Typeface.BOLD);setPadding(0,dp(8),0,dp(8))};parent.addView(v,lp())}
